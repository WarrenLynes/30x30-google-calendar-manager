...
export class GoogleCalendarService {
  ...
  all() {
    return this.httpClient.get('/api/me/calendars/primary/events').pipe(
      map((x: any) => x.items)
    )
  }

  create(model) {
    return this.httpClient.post('/api/me/calendars/primary/events', model).pipe(
      map((x: any) => ({...x, id: x.resourceName}))
    );
  }

  getUrlForId(id) {
    return `/api/me/calendars/primary/events/${id}`;
  }
